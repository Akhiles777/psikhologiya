...
export function PsychologistCard({ psychologist }: PsychologistCardProps) {
  const { slug, fullName, city, mainParadigm, certificationLevel, shortBio, price, images } =
    psychologist;
  const imageSrc = images[0] ?? null;

  return (
    <Card glass padding="none" className="overflow-hidden">
      <Link href={`/psychologist/${slug}`} className="block">
      ...
      <CardContent className="p-4">
        <Link href={`/psychologist/${slug}`} className="font-display font-semibold text-lg text-foreground hover:text-primary">
          {fullName}
        </Link>
